import Phaser, { NONE, Utils } from 'phaser';
...
export default class StatGameObject extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, texture: string, stat:Stat) {
    ...
    const text_type = scene.add.text(x - text_value_offset, y - y_text_offset, this.convert_to_short_string(stat.type), 
      { fontFamily: 'Mochiy Pop One', fontSize: '14px', color:this.convert_to_type_to_color(stat.type), stroke: '##8B661E', strokeThickness: 1 },
    );
    text_type.name = 'text_stat_type';
    this.add(text_type);
    const text_value = scene.add.text(x, y - y_text_offset, stat.value.toString(), { fontFamily: 'Mochiy Pop One', fontSize: '14px', color:'#fff', stroke: '#8B661E', strokeThickness: 1 }).setOrigin(0.5, 0);
    text_value.name = 'text_stat_value';
    this.add(text_value);
    const txt_upgrade = scene.add.text(text_value.x + 30, text_value.y, '(+1)', { fontFamily: 'Mochiy Pop One', fontSize: '14px', color:'#8CFF32', stroke: '#8B661E', strokeThickness: 1, align:'left' }).setOrigin(0.5, 0);
    txt_upgrade.name = 'txt_upgrade';
    const txt_upgrade_percent = scene.add.text(txt_upgrade.x + 100, txt_upgrade.y, '(1%)', { fontFamily: 'Mochiy Pop One', fontSize: '14px', color:'#8CFF32', stroke: '#8B661E', strokeThickness: 1, align:'left' }).setOrigin(0.5, 0);
